...
Gibbs_zeta <- function(p, a, b, psi) {
  ...
  zeta <- 1 / pmax(rgamma(p, shape = a + b, rate = rate_zeta), .Machine$double.xmin)
  bad_idx <- which(is.na(zeta))
  if (length(bad_idx) > 0) zeta[bad_idx] <- .Machine$double.xmax
  return(zeta)
}
...
Gibbs_psi <- function(p, b, phi, beta, zeta) {
  ...
  psi <- 1 / pmax(rgamma(p, shape = b + 0.5, rate = rate_psi), .Machine$double.xmin)
  bad_idx <- which(is.na(psi))
  if (length(bad_idx) > 0) psi[bad_idx] <- .Machine$double.xmax
